...
struct Node *_removeRightMost(struct Node *cur)
{
	...
	struct Node *temp = _rightMost(cur);

	TYPE x = temp->val;

	while (temp2->right->val != x)
	{
		temp2 = temp2->right;
	}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	if (cur == NULL) // empty tree
		return NULL;
	else if (val < cur->val)		// value is less than node's number. so go to left subtree
	{
		cur->left = _removeNode(cur->left, val);
	}
	else if (val > cur->val)// value is greater than node's number. so go to right subtree
	{
	...
	else // node found. Let's delete it!
	{
		...
		else // node has two children
		{
			...
			cur->left = _removeNode(cur->left, tempNode->val);		// Problem
		}
...
/* FIXME: Complete this implementation */
	...
	assert(iter);
	iter->linkedList = createLinkedList();
	iter->cur = tree->root;
	iter->end = malloc(sizeof(struct Node));
	return iter;
}
...
/* FIXME: Complete this implementation */
	assert(iter != NULL);
	deleteLinkedList(iter->linkedList);
	free(iter->end);
	free(iter);
}
...
int BSTIteratorHasNext(struct BSTreeIterator* iter) {
	/* FIXME: Complete this implementation */
	assert(iter != NULL);
	if (iter->cur == NULL && isEmptyLinkedList(iter->linkedList))
	{
		return 0;
	}
	else {
...
int BSTIteratorNext(struct BSTreeIterator* iter) {
	...
	while (counter == 0)
	{
		if (iter->cur != NULL)
		{
			pushLinkedList(iter->linkedList, iter->cur);
			iter->cur = iter->cur->left;
		}
		else {
			if (!isEmptyLinkedList(iter->linkedList))
			{
				iter->cur = topLinkedList(iter->linkedList);
				popLinkedList(iter->linkedList);
				value = iter->cur->val;
