...
void init_student(struct student* student, char* name, int id, float gpa) {
	char* newname = malloc(sizeof *newname * (strlen(name) + 1));
	strcpy(newname, name);
...
struct student* deep_copy_student(struct student* student) {
	struct student *copy = malloc(sizeof * copy);
...
struct student* find_max_gpa(struct student* students, int num_students) {
	...
	highestgpastudent = &students[0];
	for (int i = 1; i < num_students; ++i)
	{
		if (highestgpastudent->gpa < students[i].gpa)
		{
...
struct student* find_min_gpa(struct student* students, int num_students) {
	struct student *lowestgpastudent;
	lowestgpastudent = &students[0];
	for (int i = 1; i < num_students; ++i)
	{
		if (lowestgpastudent->gpa > students[i].gpa)
		{
...
void sort_by_gpa(struct student* students, int num_students) {
	...
	do {
		...
		for (int i = 1; i < num_students; ++i)
		{
			...
			{
				swapped = true;
				temp = deep_copy_student(&students[i - 1]);
				free_student(&students[i - 1]);
				init_student(&students[i - 1], students[i].name, students[i].id, students[i].gpa);
				free_student(&students[i]);
				init_student(&students[i], temp->name, temp->id, temp->gpa);
				free_student(temp);
				free(temp);
			}
		}
